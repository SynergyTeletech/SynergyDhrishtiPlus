...
public class TxnCommand {
    ...
    public TxnCommand(String[] commandsList,AsyncSocket socket,  OnAllCommandCompleted onAllCommandCompleted) {
    ...
    public TxnCommand doCommandChaining() {

        try {
            if (commandsList!=null){
                if (commandsList.length>currentCommand){
                    if (executeCommandOneByOne(commandsList[currentCommand],socket)) {
                        try {
                            Log.d("stop",commandsList[currentCommand]);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        Log.d("CommandInQueue2", "Cleared Phase One by " + Commands.getEnumByString(String.valueOf(commandsList[currentCommand])) + "" + ",Incrementing Value");
                        currentCommand++;

                        isReceived = false;
                        if (currentCommand > commandsList.length - 1) {
                            onAllCommandCompleted.commandsAllQueueEmpty(true, receive);
                            Log.d("CommandInQueue3", String.valueOf(Commands.getEnumByString(String.valueOf(commandsList[currentCommand - 1])) + "length= " + commandsList.length));
                            return null;
                        }
                        else
                        {
                            Log.d("CommandInQueueE","error");

                        }
                    }
                    else {
                        Log.d("CommandInQueueE","error1");
                    }
                }
            }
        }
    ...
    private synchronized boolean sendCommand(String command,AsyncSocket socket) {
        ...
        try {
            ...
            return new TxnCommand.MyTask(socket).execute().get();
