...
public class WriteCommandUtils {
    ...
    public WriteCommandUtils(String command,AsyncSocket socket,  OnSingleCommandCompleted onAllCommandCompleted) {

        this.Command = command;
        this.onAllCommandCompleted = onAllCommandCompleted;
        this.socket=socket;
        Log.d("TC",command +","+ socket);
        currentCommand = 0;
    ...
    private synchronized boolean sendCommand(String command,AsyncSocket socket) {
        Log.d("commandsss",""+command);
        Log.d("commandsssbyte",""+hexStringToBytes(command).toString());
...
//                  if(Command.contains(Commands.get_read_transaction_log.toString())){
//                      onAllCommandCompleted.onSingleCommandCompleted(Command,receive,socket1);
//
//                  }
...
//
                    ...
                    Log.d("Command",""+encodeHexString(byteData));
                    Log.d("CommandInQueuebytedata",""+byteData);
                    isReceived=true;
